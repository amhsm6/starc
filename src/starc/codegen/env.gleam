...
pub fn insert_symbol(env: Environment, id: ast.Identifier, sym: Symbol) -> Environment {
...
pub fn resolve_type(env: Environment, typeid: ast.TypeIdentifier) -> Result(ast.Type, SemanticError) {
...
pub fn resolve_symbol(env: Environment, id: ast.Identifier) -> Result(Symbol, SemanticError) {
  list.find_map(env.frames, fn(f) { dict.get(f.symbols, id.name) })
  |> result.replace_error(UnknownSymbol(id))
}

pub fn assert_unique_symbol(env: Environment, id: ast.Identifier) -> Result(Nil, SemanticError) {
...
pub fn assert_unique_type(env: Environment, typeid: ast.TypeIdentifier) -> Result(Nil, SemanticError) {
